...
PUBLIC void dump_fd_graph(const char * fmt, ...)
{
	...
	for (i = 0; i < NR_TASKS + NR_PROCS; i++,p_proc++) {
		...
		if ((i == TASK_TTY) || (i == TASK_SYS) || (i == TASK_HD) || (i == callerpid)) continue;
	...
	for (i = 0; i < NR_TASKS + NR_PROCS; i++,p_proc++) {
		...
		logbufpos += sprintf(logbuf + logbufpos,
				     "\t\t\"proc%d\" [\n\t\t\tlabel = \"<f0>%s (%d) |<f1> p_flags:%d(%s)|<f2> p_parent:%d%s|<f3> eip:0x%x",
				     i, p_proc->name, i, p_proc->p_flags, proc_flags[p_proc->p_flags], p_proc->p_parent,
				     p_proc->p_parent == NO_TASK ? "(NO_TASK)" : "", p_proc->regs.eip);
	...
	for (i = 0; i < NR_FILE_DESC; i++) {
		...
		logbufpos += sprintf(logbuf + logbufpos, "\t\t\tlabel = \"<f0>filedesc %d|<f1> fd_mode:%d|<f2> fd_pos:%d|<f3> fd_cnt:%d|<f4> fd_inode:%d",
				     i, f_desc_table[i].fd_mode, f_desc_table[i].fd_pos, f_desc_table[i].fd_cnt, inode_tbl_idx);
		fim[fim_idx].desc = i;
	...
	for (i = 0; i < NR_INODE; i++) {
		...
		logbufpos += sprintf(logbuf + logbufpos, "\t\t\tlabel = \"<f0>inode %d|<f1> i_mode:0x%x|<f2> i_size:0x%x"
				     "|<f3> i_start_sect:0x%x|<f4> i_nr_sects:0x%x|<f5> i_dev:0x%x|<f6> i_cnt:%d|<f7> i_num:%d",
				     i, inode_table[i].i_mode, inode_table[i].i_size, inode_table[i].i_start_sect,
				     inode_table[i].i_nr_sects, inode_table[i].i_dev, inode_table[i].i_cnt, inode_table[i].i_num);
		logbufpos += sprintf(logbuf + logbufpos, "\t\"\n");
		logbufpos += sprintf(logbuf + logbufpos, "\t\t\tshape = \"record\"\n");
		logbufpos += sprintf(logbuf + logbufpos, "\t\t];\n");
	}
	...
	for (i = 0; i < sb->nr_smap_sects; i++) { /* smap_blk0_nr + i : current sect nr. */
		...
		for (j = 0; j < SECTOR_SIZE; j++) {
			for (k = 0; k < 8; k++) {
				if (!smap_flag) {
					if ((_buf[j] >> k ) & 1) { smap_flag = 1; bit_start = (i * SECTOR_SIZE + j) * 8 + k;}
					else { continue;}
				}
				else {
					if ((_buf[j] >> k ) & 1) { continue;}
					else {
						smap_flag = 0;
						int bit_end = (i * SECTOR_SIZE + j) * 8 + k - 1;
						logbufpos += sprintf(logbuf + logbufpos, "\t\t\"sector %xh\" [\n", bit_start);
						logbufpos += sprintf(logbuf + logbufpos, "\t\t\tlabel = \"<f0>sect %xh-%xh",bit_start,bit_end);
						logbufpos += sprintf(logbuf + logbufpos, "\t\"\n");
						logbufpos += sprintf(logbuf + logbufpos, "\t\t\tshape = \"record\"\n");
						logbufpos += sprintf(logbuf + logbufpos, "\t\t];\n");
					}
	...
	for (i = 0; i < nr_dir_blks; i++) {
		...
		for (j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			if (pde->inode_nr) {
				...
				logbufpos += sprintf(logbuf + logbufpos, "\t\t\tlabel = \"<f0> %d|<f2> %s", pde->inode_nr, filename);
				logbufpos += sprintf(logbuf + logbufpos, "\t\"\n");
				logbufpos += sprintf(logbuf + logbufpos, "\t\t\tshape = \"record\"\n");
				logbufpos += sprintf(logbuf + logbufpos, "\t\t];\n");
	...
	for (i = 0; i < msd_idx; i++) {
		if (msd[i].dir == RECEIVING) logbufpos += sprintf(logbuf + logbufpos, "\t\"proc%d\":f0 -> \"proc%d\":f0 [arrowhead=\"crow\", color=\"green\", label=\"%d\"];\n", msd[i].dst, msd[i].src, i);
		else if (msd[i].dir == SENDING) logbufpos += sprintf(logbuf + logbufpos, "\t\"proc%d\":f0 -> \"proc%d\":f0 [arrowhead=\"vee\", color=\"blue\", label=\"%d\"];\n", msd[i].src, msd[i].dst, i);
		else assert(0);
	}
#endif
	...
	for (i = 0; i < NR_TASKS + NR_PROCS; i++,p_proc++) {
		if (p_proc->p_flags == FREE_SLOT) continue;
		if ((i == TASK_TTY) || (i == TASK_SYS) || (i == TASK_HD)  || (i == getpid())) continue;
		p_proc->ticks = tcks[i];
